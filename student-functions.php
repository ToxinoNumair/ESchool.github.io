		...
		<div id="event_add" class="modal">
		  <div class="modal-content">
		    <span class="close"><a href="#" onclick="close_popup('event_add')">×</a></span>
		            ...
		            <p><b>Event Title: </b><input type="text" id="eventTitle" value="" placeholder="Event Title"/></p>
			...
			// this is for create calendra and view add event and view event and number of Events
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $current_Month_First_Day+1 || $current_Month_First_Day == 7) && $cb <= ($total_Days_ofMonthDisplay)){
						...
						$result = $db->query("SELECT title FROM studentevents WHERE date = '".$currentDate."' AND status = 1");
...
function get_events_information($date = ''){
	...
	$result = $db->query("SELECT title FROM studentevents WHERE date = '".$date."' AND status = 1");
...
function add_event_information($date,$title){
	...
	$insert = $db->query("INSERT INTO studentevents (title,date,created,modified) VALUES ('".$title."','".$date."','".$currentDate."','".$currentDate."')");
	if($insert){
		echo 'ok';
	}else{
		echo 'err';
